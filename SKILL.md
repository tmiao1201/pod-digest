---
name: pod-digest
description: |
  科技播客炼金炉：任意播客 → 带时间戳逐字稿 → 结构化 digest（产业情报/科技原理/金句/术语表）
  → 三个自包含素材块（知识卡片分镜/产业链四元组/金句精选）。下游为可选放大器：本地装了 Ted-imgstyle/chain-rotation 自动接入，没装则用内置卡片风格预设与数据自助指引，开箱即用。
  触发词：「digest 这期播客」「炼一下硅谷101最新一期」「播客digest」「pod-digest」
  「提取播客逐字稿」「播客转写总结」「这期播客讲了什么值得听的」。
  适用：任意 RSS 分发的播客（中英文皆可）；有对应播客单集的 YouTube 视频（--youtube 链接须来自播客频道的一集，标题/日期与 RSS 单集互证）。
  不适用：Spotify 独家且无 YouTube 版（两个源都没有才跳过）；微信读书/文章（留给 v2）；无播客单集对应的独立 YouTube 视频（vLog/发布会，留给 v2）；只要下载音频不需要 digest。
---

# pod-digest — 科技播客炼金炉

> 一手产业信息往往先出现在播客访谈里（企业高管/创始人亲自发声），而不是研报里。
> 本 skill 把「听播客」变成可积累的情报资产：逐字稿 → digest → 素材块 → 下游框架。

## 核心纪律（先读这个）

1. **字幕优先，转写兜底**——节目有 YouTube 版先抓字幕（秒级零成本），纯音频才转写（详见 `references/acquisition.md`）
2. **Hard Stop**——**同一目标**失败 3 次立即停（换命令/参数/工具=同一目标，不重置计数），每次失败留报错原文；无报错证据的「失败」不计入、也不得作为换第 4 种姿势的理由，直接报「需人工介入」
3. **digest 是情报不是复述**——每条产业信息必须带证据强度标签和利益相关标注；嘉宾谈自家产品 = 立场存疑
4. **金句必须原话回对**——从 transcript 逐字摘（省略号拼接允许但每段须逐字命中），不润色不编造；**说话人归属核对命中处上下文**（E249 教训：子串命中≠归属正确）；`[?]` 单句≤2处、全库≤30%；回对不上的一律降级为不带引号的转述
5. **输出恒为中文**（转写层保留原文，翻译在 digest 层做，规则见 `references/digest-format.md`）

## Common Rationalizations（反偷懒表）

| Agent 可能的借口 | 为什么这是错的 |
|-----------------|---------------|
| "YouTube 拿字幕失败，直接转写吧" | 字幕路径**零成本零 key**，转写烧配额。必须先 `--list-subs` 确认真无字幕（实测：E250 新传 1 天无字幕，E249 有人工字幕——同一频道两种情况都存在）。列表显示有字幕但下载失败才转转写 |
| "金句凭印象写，意思对就行" | E249 实测「AGI 是 marketing term」这句极易张冠李戴到张宏江头上（实际是黄东旭说的）。金句 100% 由 verify_digest.py 机检回对（非抽 2 条）；归属人工核对命中处上下文，确认不了标 [?] 且逐出素材块③ |
| "利益相关标一下就行" | 只写「利益相关」四个字等于没写。必须写明具体利害（E250 样板：「老东家+竞品+自家管线」三重），读者才能校准折扣力度 |
| "这一步失败了，换个姿势再试试" | Hard Stop：同一目标失败 3 次即停（换命令/参数/工具=同一目标）。实测：TLS 指纹 403 换 3 种姿势全无效。失败须附报错原文——无报错的「失败」是弃疗不算数 |
| "四元组表凑几行 A 股标的，让产业链分析能跑起来" | chain-rotation 红线：绝不编数字、不借嘉宾之口造事实。E250/Acquired 两期均如实判「A 股映射弱」——诚实标注比硬凑有价值，延伸方向可自研另跑 |

## 使用流程

### Step 0：环境体检（首次/报错时）

```bash
# 转写引擎可用性（字幕路径不需要任何 key）
ls ~/.local/bin/agent-reach 2>/dev/null && agent-reach doctor --json | head -30
echo "GROQ_API_KEY=${GROQ_API_KEY:+已设置}"
```

两个都没有 → 只能走字幕路径，遇到纯音频节目明确告知用户怎么配（见 `.env.example`），不要卡死。

### Step 1：获取逐字稿

```bash
cd ~/cc/pod-digest
# 常见三种入口
python3 scripts/get_transcript.py "硅谷101" --match Moderna          # 按关键词选集
python3 scripts/get_transcript.py "硅谷101" --latest                  # 最新一期
python3 scripts/get_transcript.py "硅谷101" --match E250 \
    --youtube "https://www.youtube.com/watch?v=xxx"                   # 有YouTube版，抓字幕免转写
```

产物：`out/episodes/<节目>/<日期>-<slug>/transcript.txt + meta.json`（meta 含 shownotes——章节目录和嘉宾信息是 digest 的重要输入）。

选集/搜索引擎/踩坑 → `references/acquisition.md`。

### Step 2：生成 digest

读 `transcript.txt` 全文 + `meta.json` 的 shownotes，按 `references/digest-format.md` 的模板写 `digest.md` 到同目录。

关键动作：
- 优先用 shownotes 的官方章节目录对齐时间戳（比 whisper 的更准）
- 产业情报逐条标证据强度（🟢有数据/🟡单一口述/🔴利益相关）
- 外文播客：digest 中文、金句双语、必出术语表
- 跳过广告/口播/社群推广段（情报型 agent 的去噪职责）

### Step 3：素材块 + 下游（半自动，两个等价选项；下游为可选放大器）

digest 末尾固定产出三块（schema 见 `references/digest-format.md`）：
① 知识卡片分镜草案 ② 产业链四元组草案 ③ 金句精选。
**素材块本身就是终点交付物**——下游 skill 有则放大、无则内置 fallback（探测逻辑与降级路径见 `references/downstream.md`）。

下游由用户一句话触发，三个选项平级：
- 用户说「**出报告**」→ `python3 scripts/make_report.py <本期目录>`：digest.md → 自包含 HTML 研究简报（企业蓝、离线可开、可打印 PDF、零依赖）——内置下游，产物 report.html 在同目录
- 用户说「**出卡片**」→ 素材块① 出成套生图提示词：本地有 Ted-imgstyle 走其风格库；没有则用 `references/card-styles.md` 三预设（G1手账/G2扁平/G3黑白）组装，产物=模型无关提示词，粘贴给任意生图模型即可
- 用户说「**接产业链**」或「**出产业链分析**」→ 素材块② ：本地有 chain-rotation 则映射成 COMPANIES 清单跑兑现轮动（数值字段留空由数据层填，**绝不编数字**）；没有则四元组表即交付，附 tushare/akshare 数据自助指引。若素材块②已判定「本期不适合」（映射弱/证据链薄），如实告知并给自研延伸方向，不硬凑

### Step 4：信封落盘

digest 完成后由 `python3 scripts/verify_digest.py <本期目录> --write-envelope` 回写 `out/envelopes/pod-digest.json`——status 由机检结果决定（全绿才 green），headline/deliverable/episode_dir 由脚本从实际产物解析，self_check 为机检摘要。**不手写信封**，手写会被下次机检覆写。

## When NOT to Use（不适用场景）

- **Spotify 独家且无 YouTube 版**：两个源都没有才跳过；有 YouTube 版走字幕路径并在 digest 标注来源为视频版（拒单/判弱前必须先完成两项查证，不许口头断言）
- **只要音频文件/集数列表**：直接跑 `parse_feed.py --list` 或下载即可，不要进 digest 全流程
- **微信读书/公众号文章/单条 YouTube 视频本身**：不是播客形态，属 v2 扩展源，pod-digest 不接
- **要"实时"**：转写数分钟 + digest 需 LLM 通读，本质是深加工工具不是实时工具

## Red Flags（执行中违规信号）

> 产物类红旗已由 `verify_digest.py` 机械化兜底（FAIL=违规成立）；行为类（重试纪律）以 meta.json `attempts` 执行记录为准——「发现/想」是提示不是依赖。

- **同一目标第 3 次失败**（换姿势不重置计数）：Hard Stop 触发，列已试方案+各自报错报「需人工介入」
- **产业情报存在无标签/无时间戳条目**（脚本 FAIL）：停下补齐再继续——不带标签的情报违反情报型 agent 的证伪纪律
- **准备往四元组填代码或数值**：停。数据层字段（代码列+估值/百分比/财务数值）只能由数据层填；**播客口述数字的唯一合法去处=产业情报原话引用**
- **没有 transcript.txt 就动笔 digest**：停——脚本第 1 步即 FAIL；宁可不交付也不编造

## Verification（退出检查）

digest 完成前逐项过，全绿才算完成：

- [ ] `python3 scripts/verify_digest.py <本期目录>` 退出码 0，退出码贴进交付报告（机检：转写覆盖勾稽/金句全量逐字符回对/时间戳锚点/四元组代码与数值正则/证据标签/语言义务）
- [ ] digest.md 六板块齐全：meta / 一句话价值 / 产业情报 / 科技原理 / 术语表 / 金句库
- [ ] **人工残余项（机检不了的三项）**：①金句说话人归属在命中处上下文核对过（确认不了标 [?] 且不进素材块③）②利益相关标注达「身份×对象×偏向」质量 ③字幕源三查的人工段（覆盖/专名/误转率）
- [ ] 「A股映射弱/无映射」判定已附查证记录行（WebSearch/tsdata + 日期）；信封由 `verify_digest.py --write-envelope` 回写——手写 status 会被下次机检覆写

## 文件地图

| 路径 | 说明 |
|---|---|
| `scripts/get_transcript.py` | 主入口：选集+下载+转写一条龙（幂等） |
| `scripts/make_report.py` | 内置下游③「出报告」：digest.md → 自包含 HTML 简报（零依赖） |
| `scripts/itunes_search.sh` / `parse_feed.py` / `transcribe.sh` / `transcribe_groq.py` | 分步工具（acquisition.md 有详解） |
| `references/acquisition.md` | 获取路由 + 8 条实测坑 + Hard Stop 纪律 |
| `references/digest-format.md` | digest 模板逐节规范 + 证据强度标签 + 素材块 schema |
| `references/downstream.md` | Ted-imgstyle / chain-rotation 衔接 + 数据源选项 |
| `references/local-setup.md` | 本地私设（发布公域时整体替换为通用安装指引） |
| `config/shows.yaml` | 订阅清单（watchlist） |
| `out/episodes/<show>/<date>-<slug>/` | transcript.txt / digest.md / meta.json |
